      ...
      if (mintLimit.pda) {
        ...
        if (mintLimit.accountInfo) {
          try {
            ...
            console.warn('Failed to parse mint counter:', error);
          }
        }
      }
  ...
  if (guards.mintLimit) {
    let canPayFor =
      typeof guards.mintLimit?.settings?.limit == "number"
        ? guards.mintLimit.settings.limit - 0
        : 10;
...
export async function getNftImageUrl(metadataUri: string): Promise<string | null> {
  try {
    ...
    // Replace Pinata gateway with configurable IPFS gateway for public access
    ...
    if (imageUrl?.startsWith(pinataGateway)) {
      imageUrl = imageUrl.replace(pinataGateway, ipfsGateway);
    }
    return imageUrl;
  } catch {
    return null;
...
export async function fetchNftMetadata(metadataUri: string): Promise<any | null> {
  try {
    ...
    console.error("fetchNftMetadata error:", e);
    return null;
  }
}
